...
def build_prompt(
    ...
    translated_before: dict | None = None,
) -> str:
    """
    ...
    if langs:
        ...
        # Try to format with num_subtitles if present in template
        try:
            formatted_instr = instr.format(
                langs=", ".join(langs),
                num_langs=len(langs),
                num_subtitles=1,  # Default to 1 for single subtitle
                format_example=format_example,
                lang_tags=lang_tags,
            )
        except KeyError:
            # Fall back if num_subtitles not in template
            formatted_instr = instr.format(
                langs=", ".join(langs),
                num_langs=len(langs),
                format_example=format_example,
        ...
        # Single language mode - handle both old and new format
        try:
            # Try new batch format first (requires num_subtitles and subtitle_list)
            formatted_instr = instr.format(num_subtitles=1, lang=lang, subtitle_list=text)
        except KeyError:
            # Fall back to old single-subtitle format
